...
private:
   ...
   std::vector<float> m_values; // vector of values
...
public:
   ...
   SparseMode(const MatrixXui32& idx, const std::vector<float>& vals, std::uint64_t mode, std::uint64_t mode_size);
...
public:
   std::pair<PVec<>, float> item(std::uint64_t hyperplane, std::uint64_t item) const;
