...
template<typename T>
int  solve_blockcg(Eigen::MatrixXf & X, T & t, float reg, Eigen::MatrixXf & B, float tol, const int blocksize, const int excess, bool throw_on_cholesky_error = false);
template<typename T>
int  solve_blockcg(Eigen::MatrixXf & X, T & t, float reg, Eigen::MatrixXf & B, float tol, bool throw_on_cholesky_error = false);

inline void AtA_mul_B(Eigen::MatrixXf & out, SparseSideInfo & A, float reg, Eigen::MatrixXf & B);
inline void AtA_mul_B(Eigen::MatrixXf & out, Eigen::MatrixXf & A, float reg, Eigen::MatrixXf & B);

inline void makeSymmetric(Eigen::MatrixXf &A)
{
...
template<typename T>
inline int solve_blockcg(Eigen::MatrixXf & X, T & K, float reg, Eigen::MatrixXf & B, float tol, const int blocksize, const int excess, bool throw_on_cholesky_error) {
...
//
template<typename T>
inline int solve_blockcg(Eigen::MatrixXf & X, T & K, float reg, Eigen::MatrixXf & B, float tol, bool throw_on_cholesky_error) {
  // initialize
  ...
  Eigen::VectorXf norms(nrhs), inorms(nrhs); 
  norms.setZero();
  inorms.setZero();
  ...
  Eigen::MatrixXf R(nrhs, nfeat);
  Eigen::MatrixXf P(nrhs, nfeat);
  Eigen::MatrixXf Ptmp(nrhs, nfeat);
  X.setZero();
  // normalize R and P:
  ...
  Eigen::MatrixXf* RtR = new Eigen::MatrixXf(nrhs, nrhs);
  Eigen::MatrixXf* RtR2 = new Eigen::MatrixXf(nrhs, nrhs);

  Eigen::MatrixXf KP(nrhs, nfeat);
  Eigen::MatrixXf PtKP(nrhs, nrhs);
  ...
  *RtR = R * R.transpose();
  ...
  for (iter = 0; iter < 1000; iter++) {
    ...
    *RtR2 = R * R.transpose();
    ...
    Eigen::VectorXf d = RtR2->diagonal();
    ...
    for (int block = 0; block < nblocks; block++) 
    {
      int col = block * 64;
      int bcols = std::min(64, nfeat - col);
      Eigen::MatrixXf xtmp(nrhs, bcols);
  ...
  if (iter == 1000)
  {
    Eigen::VectorXf d = RtR2->diagonal().cwiseSqrt();
