...
void ScarceMatrixData::init_pre()
{
   ...
   for(unsigned i=0; i<Ycentered->size(); ++i)
   {
      auto &v = Ycentered->operator[](i);
...
double ScarceMatrixData::compute_mode_mean_mn(int mode, int pos)
{
   assert(Ycentered);
    ...
    const auto &col = Ycentered->at(mode).col(pos);
    if (col.nonZeros() == 0)
      return getCwiseMean();
